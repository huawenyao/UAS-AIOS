...
2. 交互必须是空间动词，不是系统平面菜单。宫殿是第一人称可走入的门厅（透视地面、墙上门洞、点地走路）；百宝箱是脚边的匣子；穿梭是把信物投进窗台（重温）或未写下的门（改写）。没有选项卡、没有单选框、没有「穿梭进入」按钮、没有横排导航拱门。退路是脚下的门槛，不是界面返回键。
3. 默认绝对私有；分享是可选导出，现实记忆不参与训练。
4. 留存靠资产沉淀。必须提供归档、封存、融合、删除，否则百宝箱会变成垃圾堆。
5. 重温与改写用空间区分：玻璃罩 = 原点只读；未写下的门 = COW 平行分支。误改真实记忆是产品事故。
