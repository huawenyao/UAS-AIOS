...
def main():
    ...
    # 价值摘要（P0 契约）
    try:
        from value_summary import build_value_summary, format_value_summary_console

        vs = build_value_summary(
            results,
            total_files=len(resume_files),
            valid_count=valid_count,
        )
        print(format_value_summary_console(vs))
        strong_recommend = [r for r in results if r.get("decision") == "strong_recommend"]
        if strong_recommend:
            print("\n强烈推荐候选人（Top 3）:")
            for r in strong_recommend[:3]:
                name = r.get("name", "未知")
    ...
    print("\n报告已生成:")
    print("   • 推荐名单: reports/recommendation_list.html")
    print("   • 个人报告: reports/candidate_*.html")
    print("   • 数据文件: database/candidates.json")
    print("\n一键入口: python run_batch.py --resume-dir <目录>")