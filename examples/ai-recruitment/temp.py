...
def parse_resume_info(text):
    ...
    if years:
        ...
        if years:
            info['experience_years'] = datetime.now().year - min(years)
...
def main(resume_directory, top_n=5):
    ...
    # 2. 处理每份简历
    print(f"\n[步骤2/5 处理简历文件，共 {len(resume_files)} 份]")
    candidates = []
    ...
    # 4. 输出结果
    print(f"\n[步骤4/5 输出匹配度前 {top_n} 名候选人]")
    print("-" * 60)
    
    for i, candidate in enumerate(candidates_sorted[:top_n], 1):
        print(f"\n第{i}名: {candidate['name']}")
        print(f"   综合得分: {candidate['total_score']}/100")
        print(f"   技能匹配: {round(candidate['detailed_scores']['skill_match'], 1)}分")
        print(f"   经验匹配: {round(candidate['detailed_scores']['experience_match'], 1)}分")
        print(f"   教育匹配: {round(candidate['detailed_scores']['education_match'], 1)}分")
        print(f"   管理能力: {round(candidate['detailed_scores']['management_match'], 1)}分")
        print(f"   行业匹配: {round(candidate['detailed_scores']['industry_match'], 1)}分")
        print(f"   核心优势:")
        for advantage in candidate['core_advantages']:
            print(f"     - {advantage}")
        print(f"   文件路径: {candidate['file_path']}")
    ...
    output_file = os.path.join(output_dir, f'ai_director_matches_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json')
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(candidates_sorted[:top_n], f, ensure_ascii=False, indent=2)
...
if __name__ == "__main__":
    # 请修改为实际的简历目录路径
    RESUME_DIRECTORY = r"C:\Users\ranwu\Documents"
