...
站外「仪式礼物」令牌；完整字段与落地契约见 [BETA_SHELL_AND_GIFT_SPEC](./BETA_SHELL_AND_GIFT_SPEC.md) §3。

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `gift_id` | string | 是 | 礼物 ID |
| `token_hash` | string | 是 | 明文 token 不落库 |
| `ritual_ref` | string | 是 | 源仪式 |
| `keepsake_ref` | string | 否 | 已保存时关联 |
| `grantor_ids` | string[] | 是 | 外发授权人；共同作品须全体 |
| `surface` | enum | 是 | MVP/Beta：`expiring_link` |
| `payload_public` | object | 是 | 标题/模态/公开封面/邀请句；**无原料** |
| `expires_at` | string | 是 | 短过期；受策略上限 |
| `max_resolves` | int | 是 | 防广播刮取 |
| `resolve_count` | int | 是 | 已解析次数 |
| `status` | enum | 是 | `active` / `expired` / `revoked` / `exhausted` |
| `share_grant_refs` | string[] | 是 | 级联撤回 |

**硬约束：** `payload_public` 不得含 signals、pulse、trace 细节；`ShareGrant` 撤回必须级联 `GiftToken.status=revoked`。
...
`mint → active → expired | revoked | exhausted`

事件：`gift.minted`、`gift.resolved`、`gift.revoked`、`gift.expired`、`gift.access_denied`。

`active` 下 `resolve` 可导致 `exhausted`；任意终态后 resolve 返回 `GIFT_INVALID` 或 `SHARE_REVOKED`。
...
7. `GiftToken.payload_public` 不得含原料；撤回共享必须级联礼物令牌失效。
8. Intent 类型可扩展 `gift_mint` / `gift_resolve`；不得因此放宽 purpose。
