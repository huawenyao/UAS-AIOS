...
> CASE-001～017 验证产品价值、主权、安全、失败恢复、演化闭环与礼物病毒。每例必须产出 state、audit、report，以及成功 `RitualEnvelope` / `GiftToken` 或明确错误/延期结果。
...
| 015 | 单人仪式→铸造礼物 | `gift_active` | 礼物病毒、无原料外发 | 令牌过期策略、原料泄漏=0 |
| 016 | 礼物落地解析与转化入口 | `gift_resolved` | 先体验后共创 | resolve 上限、无推荐流 |
| 017 | 礼物/共享撤回级联 | `GIFT_INVALID`/`SHARE_REVOKED` | 共同主权 | 撤回后访问=0 |
...
## CASE-015 · 有意义仪式后铸造礼物令牌

```yaml
case_id: CASE-015
intent_type: gift_mint
person_id: person_ada
precondition:
  ritual_id: ritual_001
  ritual_state: revealed
  feedback: meaningful
  owners: [person_ada]
gift_request:
...
- `lw.gift.token.mint` 成功；明文 token 仅出现一次；库内仅 `token_hash`。
- `payload_public` 审计/导出中**不含** signals、pulse、trace 细节。
- 不调用任何 IM/邮件代发能力；仅返回 `resolve_url`。
- 终态 `gift_active`；关联 `ShareGrant.surface=expiring_link`。
- 追溯：本质文档“礼物病毒” → F2/F6/F7；KPI：原料泄漏=0、M-11 相关。

## CASE-016 · 礼物落地解析（无原料、有限次）

```yaml
case_id: CASE-016
intent_type: gift_resolve
token_ref: from_CASE_015
max_resolves: 2
resolve_attempts: 2
```
...
- 每次成功 resolve 返回 `public_card` + CTA `[create_for_moment, invite_duet_with_grantor, leave]`。
- 无推荐流、无「再看更多」；DOM/响应无原料字段。
- 第 2 次后状态 `exhausted`；再 resolve → `GIFT_INVALID`。
- 追溯：TS-P3 / SHELL-08；KPI：非法字段泄漏=0。

## CASE-017 · 共享撤回级联礼物失效

```yaml
case_id: CASE-017
intent_type: gift_revoke_cascade
precondition:
  shared_keepsake: true
  gift_status: active
  owners: [person_ada, person_lee]
action:
  actor: person_lee
  call: lw.share.revoke
```

**期望**

- Keepsake 共享与 `GiftToken` 均进入撤回/失效；落地 resolve → `SHARE_REVOKED` 或 `GIFT_INVALID`。
- 双方获得各自 receipt；不披露撤回理由。
- 生效时延计入 M-09；成功计入 M-11。
- 追溯：宪章关系编织 → F2；CASE-010 的礼物面扩展。
...
- CASE-015～017 未实现前，Beta Gift Edge 不得宣称可外发；实现后须与 CASE-001～014 一并回归。
