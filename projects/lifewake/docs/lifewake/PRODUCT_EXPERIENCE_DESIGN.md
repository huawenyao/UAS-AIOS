...
完整 IA 与令牌语义见 [BETA_SHELL_AND_GIFT_SPEC](./BETA_SHELL_AND_GIFT_SPEC.md)。

| 阶段 | 用户看见 | 用户动作 | 系统动作 | 体验验收 |
|---|---|---|---|---|
| 发起 | 「做成礼物送给 TA」；明示对方看不到原料 | 确认外发 | `lw.gift.token.mint` | 共同作品须双方确认 |
| 外发 | 可复制链接 | 用户手动分享 | 不代发 | 无静默 IM |
| 落地 | 公开卡片体验 + 三个 CTA | 体验/共创/离开 | `lw.gift.token.resolve` | 无推荐流、无原料 |
| 撤回 | 权利占位 | 权利人撤回 | token + share 级联失效 | CASE-017 |
...
| UX-13 | Home 非功能宫格；首路径为击穿 CTA | SHELL-01 |
| UX-14 | 礼物 public 载荷无原料；revoke 后落地失效 | CASE-015～017 |
| UX-15 | 礼物转化 CTA 不含推荐流或羞耻召回 | Gift Landing 审查 |

体验 CASE 与工程 CASE 的映射见 [DERIVATION_AND_VALIDATION_MATRIX](./DERIVATION_AND_VALIDATION_MATRIX.md)。  
Beta Shell / Gift 规格见 [BETA_SHELL_AND_GIFT_SPEC](./BETA_SHELL_AND_GIFT_SPEC.md)。
