...
| `gift.minted` | Gift Compose | gift_id、expires_bucket、max_resolves | CASE-015 |
| `gift.resolved` | Gift Landing | result_code、remaining_resolves | CASE-016 |
| `gift.revoked` | Gift/Vault | enforced_at、cascaded_share | CASE-017、M-11 |
...
| E7 礼物邀请 | 有意义仪式后一键礼物提高双方自愿共创 | 有/无礼物 CTA | 对方 create/duet 率、MRCR | 原料泄漏、催促投诉、撤回失败 | 护栏全绿才扩量 |
...
> 增长哲学的完整推导（单点击穿、礼物病毒、反成瘾）见 [PRODUCT_ESSENCE_CARRIER_AND_TECH](./PRODUCT_ESSENCE_CARRIER_AND_TECH.md) §2–§3。
...
传播原子是「可过期、无原料、可撤回的仪式礼物」，不是信息流或打卡召回。优先实验：单人有意义仪式完成后 → 一键生成双人礼物邀请。
