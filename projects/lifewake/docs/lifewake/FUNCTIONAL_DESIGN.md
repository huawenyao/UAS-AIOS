...
| Growth/Edge | F-GIFT | 仪式礼物 mint/resolve/revoke（无原料、可过期） | 具名邀请、礼赠订阅包装 | 跨通道礼物协议 |
...
## 12A. F-GIFT · 仪式礼物（Beta）

**用户故事**：作为刚完成有意义仪式的用户，我希望把一件**无原料**的作品做成可过期礼物送给一个人，并随时撤回；对方先体验再选择是否共创。

| 项 | 规格 |
|---|---|
| 前置 | 仪式已 `revealed`；非低冲击未交付；共同作品须全体 owners 确认外发 |
| 输入 | `ritual_ref`、`public_card`、`expires_at`、`max_resolves`、grantor 确认 |
| 处理 | `lw.gift.token.mint` → 用户手动复制链接 → `resolve` / `revoke` |
| 后置 | `GiftToken` active；关联 ShareGrant；明文 token 仅一次 |
| 异常 | 原料泄漏检测、`GIFT_SOURCE_NOT_REVEALED`、`GIFT_COOWNER_UNCONFIRMED`、过期/用尽/撤回 |
| 验收 | CASE-015～017；SHELL-07/08；禁止静默代发与推荐流 CTA |
| 追溯 | 本质文档礼物病毒；F2/F6/F7；[BETA_SHELL_AND_GIFT_SPEC](./BETA_SHELL_AND_GIFT_SPEC.md) |
